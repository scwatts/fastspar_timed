...
g <- g + xlim(0, 2500)
g <- g + labs(x='OTU number', y='Time (minutes)', colour='Method')
g
}
dev.off()
...
# Summarised
png(filename='plots/comparison_summary_time.png', height=1000, width=1400, res=160)
{
  g <- ggplot(d.time.melt, aes(x=factor(OTUs), y=time, colour=Method)) + geom_boxplot(width = 0.25)
  g <- g + scale_colour_manual(values=c("#F8766D", "#00BA38", "#619CFF"))
  g <- g + labs(x='OTUs', y='Time (minutes)')
  g
}
dev.off()

png(filename='plots/comparison_summary_memory.png', height=1000, width=1400, res=160)
{
