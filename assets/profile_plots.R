...
fastspar.time.lm <- lm(sqrt(d.time$`FastSpar (single thread)`) ~ d.time$OTUs + d.time$Samples)
summary(fastspar.memory.lm)
summary(fastspar.time.lm)


### Plot
svg(filename='plots/comparison_full_time.svg', height=8, width=16)
{
...
svg(filename='plots/comparison_full_memory.svg', height=8, width=16)
{
